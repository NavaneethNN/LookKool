/** @type {import('next').NextConfig} */
const nextConfig = {
  // Supabase Storage images
  images: {
    remotePatterns: [
      {
        protocol: "https",
        hostname: "*.supabase.co",
        pathname: "/storage/v1/object/public/**",
      },
    ],
    // Cache optimized images for 30 days (CDN) instead of default 60s
    minimumCacheTTL: 2592000,
  },

  // Security + caching headers
  async headers() {
    return [
      {
        // Security headers for all routes
        source: "/(.*)",
        headers: [
          { key: "X-Frame-Options", value: "DENY" },
          { key: "X-Content-Type-Options", value: "nosniff" },
          { key: "Referrer-Policy", value: "strict-origin-when-cross-origin" },
          {
            key: "Permissions-Policy",
            value: "camera=(), microphone=(), geolocation=()",
          },
          {
            key: "Strict-Transport-Security",
            value: "max-age=63072000; includeSubDomains; preload",
          },
        ],
      },
      {
        // Long-lived cache for static assets (fonts, images in public/)
        source: "/fonts/:path*",
        headers: [
          {
            key: "Cache-Control",
            value: "public, max-age=31536000, immutable",
          },
        ],
      },
      {
        // Cache Next.js static chunks (hashed filenames = immutable)
        source: "/_next/static/:path*",
        headers: [
          {
            key: "Cache-Control",
            value: "public, max-age=31536000, immutable",
          },
        ],
      },
    ];
  },

  // Minimize output
  poweredByHeader: false,

  // Prevent the Router Cache from going stale at 5 minutes
  // (default staleTimes.static = 300s causes RSC refetches that can
  // conflict with in-flight server action calls → frozen UI)
  experimental: {
    staleTimes: {
      dynamic: 30,
      static: 180, // keep static Router Cache for 3 minutes, then soft-refresh
    },
  },
};

export default nextConfig;
